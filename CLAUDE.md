...
- **Lint client**: `cd client-nuxt && yarn lint` (eslint)
...
- **Tests**: a minimal Go suite lives in `server/*_test.go` (e.g. `bridge_test.go` — call segmenter + UDP rebind race); run with `cd server && go test ./...`. No client test suite.
...
## Boundaries
- **Rebuild the client before the server.** `server/webapp/` is generated by `yarn build` and embedded at `go build` time; it is gitignored and can drift. A stale bundle = runtime errors absent from source. Frontend change → `yarn build` → `go build`.
- **Raw SQL is built with `fmt.Sprintf`, not bound parameters.** Any free-form string in a query must go through `escapeQuotes()` or it breaks / injects on apostrophes. `migrations.go` is still raw.
- **Never commit secrets** — admin passwords, API keys, or a config INI containing them.
- **`setup.sh` is the deployment entry point** on the Pi (build from source + seed config + systemd units); keep it in step with server/config changes.
